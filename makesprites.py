...
class SpriteTask():
    ...
    def takesnaps(self, videofile,newoutdir,thumbRate=None) -> list:
        """
    ...
    def get_thumb_images(self, newdir):
    ...
    def get_geometry(self, file) -> str:
        ...
        geom = self.doCmd("""identify -format "%%g - %%f\n" %s""" % pipes.quote(file))
        parts: list = geom.split("-", 1)
        return parts[0].strip() #return just the geometry prefix of the line, sans extra whitespace

    def makevtt(self, spritefile,numsegments,coords,gridsize,writefile,thumbRate=None):
    ...
    #==== END SAMPLE ========
        ...
        vtt =  "\n".join(vtt)
        #output to file
        self.writevtt(writefile,vtt)

    def get_time_str(self, numseconds,adjust=None):
    ...
    def makesprite(self, outdir, spritefile: str, coords: str, gridsize):
    ...
    def run(self, thumbRate=None):
        ...
        #create snapshots
        numfiles,thumbfiles = self.takesnaps(self.getVideoFile(),outdir, thumbRate=thumbRate)
        #resize them to be mini
        self.resize(thumbfiles)

        #get coordinates from a resized file to use in spritemapping
        gridsize = int(math.ceil(math.sqrt(numfiles)))
        coords: str = self.get_geometry(thumbfiles[0]) #use the first file (since they are all same size) to get geometry settings

        #convert small files into a single sprite grid
        self.makesprite(outdir,spritefile,coords,gridsize)
